...
object ods_taobao_user_behavior {
  def main(args: Array[String]): Unit = {
    ...
    spark.sql(
      """
        |CREATE EXTERNAL TABLE if not exists taobao_user_behavior (
        |user_id       INT,
        |cmd_id       INT,
        |cmd_type_id  INT,
        |behavior     STRING,
...
//    spark.sql(
//      s"""
//        |LOAD DATA INPATH '/data_log/$formattedDate/*'  INTO TABLE taobao_user_behavior;
//        |""".stripMargin)
//    // hdfs://bigdata1:8020/user/hive/warehouse/ods.db/taobao_user_behavior/
//println("写入完成")
    spark.sql("select * from ods.taobao_user_behavior limit 5").show(false)
